...
export function ProveedoresTable({
  ...
  const columns: GridColDef<Proveedor>[] = [
    { field: 'id_proveedor', headerName: 'ID', width: 80 },
    { field: 'nombre_proveedor', headerName: 'Nombre', flex: 1, minWidth: 220 },
        ...
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          ...
          <Tooltip title="Desactivar">
            <IconButton
              ...
              color="error"
              disabled={deleting}
              onClick={() => onAskDelete([params.row.id_proveedor])}
            >
              <IconTrash size={18} />
            </IconButton>
          </Tooltip>
    ...
    <Box sx={{ width: '100%' }}>
      {selectedIds.length > 0 && (
        <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
          <Tooltip title="Desactivar selección">
