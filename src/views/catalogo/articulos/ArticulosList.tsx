...
export default function ArticulosList({ items, onEdit, onAskDelete, canEdit = true, canDelete = true }: Props) {
  const [menuAnchor, setMenuAnchor] = useState<HTMLElement | null>(null);
  const [menuRow, setMenuRow] = useState<Articulo | null>(null);

  const openMenuFor = (el: HTMLElement, row: Articulo) => {
    setMenuAnchor(el);
    setMenuRow(row);
  };
  const closeMenu = () => {
