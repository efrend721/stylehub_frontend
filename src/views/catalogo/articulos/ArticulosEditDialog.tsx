...
import type { Articulo, UpdateArticuloPayload, Option } from './types';
...
function buildUpdatePayload(
  ...
  perms: {
    canUpdateCosto: boolean;
    canUpdatePrecio: boolean;
    canUpdateFraccion: boolean;
  }
  ...
  return {
    ...
    fraccion: perms.canUpdateFraccion ? fraccionNum : undefined,
    costo: perms.canUpdateCosto ? costoNum : undefined,
    precio: perms.canUpdatePrecio ? precioNum : undefined,
    // Ajustes manuales: sólo si el usuario puede cambiar el campo base
    costo_fraccion: includeCostoFraccion && perms.canUpdateCosto ? costoFraccionValue : undefined,
    precio_fraccion: includePrecioFraccion && perms.canUpdatePrecio ? precioFraccionValue : undefined,
...
function buildCompareSnapshot(
  ...
  return JSON.stringify(buildUpdatePayload(source, precioText, fraccionText, costoText, includeCostoFraccion, includePrecioFraccion, perms));
}
...
interface Props {
  ...
  saving: boolean;
  onClose: () => void;
  onSave: (payload: UpdateArticuloPayload) => void;
...
export function ArticulosEditDialog({ item, saving, onClose, onSave, fieldErrors, tipoOptions, categoriaOptions, proveedorOptions, onSearchProveedor }: Props) {
  const open = !!item;
  const { user } = useAuth();
  ...
  const isDirty =
    !!form &&
    initialSnapshotRef.current !== null &&
    buildCompareSnapshot(form, precioInput, fraccionInput, costoInput, includeCostoFraccion, includePrecioFraccion, perms) !== initialSnapshotRef.current;
  ...
  const handleSave = () => {
    ...
    const payload: UpdateArticuloPayload = buildUpdatePayload(
      {
        ...form,
        precio: precioNum,
  ...
  return (
    <Dialog 
      ...
      maxWidth="sm" 
      fullWidth
      aria-labelledby="edit-articulo-dialog-title"
    >
      <DialogTitle id="edit-articulo-dialog-title">Editar Artículo</DialogTitle>
      {form && (
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setForm((prev) => {
                      if (!prev) return prev;
                      const next: Articulo = { ...prev, costo: Number.isNaN(costo) ? prev.costo : costo };
                  ...
                  disabled={saving || !canUpdateCosto}
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setForm((prev) => {
                      if (!prev) return prev;
                      const next: Articulo = { ...prev, fraccion: Number.isNaN(fraccion) ? prev.fraccion : fraccion };
                  ...
                  disabled={saving || !canUpdateFraccion}
                  slotProps={{ htmlInput: { min: 0 } }}
                  fullWidth
                />
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setForm((prev) => {
                      if (!prev) return prev;
                      const next: Articulo = { ...prev, precio: Number.isNaN(precio) ? prev.precio : precio };
                      if (!manualPrecioFraccion) {
                        if (next.fraccion && next.fraccion >= 1) {
                          next.precio_fraccion = Math.round(next.precio / next.fraccion);
                        } else {
                  ...
                  disabled={saving || !canUpdatePrecio}
                  slotProps={{ htmlInput: { min: 0 } }}
                  fullWidth
                  required
                />
