...
import { TiposProductoService } from '#/services';
import { notify } from '#/utils/notify';
import { getErrorMessage, getErrorStatus, getErrorArray } from '#/utils/errorUtils';
import type { TipoProducto, CreateTipoProductoPayload, UpdateTipoProductoPayload } from './types';
...
export function useTiposProducto() {
  const [rows, setRows] = useState<TipoProducto[]>([]);
  ...
  const [editItem, setEditItem] = useState<TipoProducto | null>(null);
  ...
  const fetchTipos = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await TiposProductoService.getAll();
      setRows(Array.isArray(data) ? data : []);
    } catch (e) {
      const msg = getErrorMessage(e, 'No se pudo cargar tipos de producto');
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const doBulkDelete = async () => {
    ...
    setDeleting(true);
    try {
      const { deleted, requested } = await TiposProductoService.deleteMultiple(deleteIds);
  ...
  const openEditFor = (item: TipoProducto) => {
    setEditItem({ ...item });
    setEditFieldErrors({});
  };
  ...
  const saveEdit = async () => {
    ...
    setEditFieldErrors({});
    try {
      const payload: UpdateTipoProductoPayload = { nombre_tipo: editItem.nombre_tipo.trim() };
      await TiposProductoService.update(editItem.id_tipo, payload);
      notify.success('Tipo de producto actualizado');
      setEditItem(null);
      await fetchTipos();
    } catch (e) {
  ...
  const createTipo = async (payload: CreateTipoProductoPayload) => {
    setCreating(true);
    setCreateFieldErrors({});
    try {
      await TiposProductoService.create({ nombre_tipo: payload.nombre_tipo.trim() });
      notify.success('Tipo de producto creado');
      closeCreateDialog();
      await fetchTipos();
    } catch (e) {
      const status = getErrorStatus(e);
